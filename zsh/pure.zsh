...
# 165392 => 1d 21h 56m 32.578s
# https://github.com/sindresorhus/pretty-time-zsh
prompt_pure_human_time_to_var() {
	local human total_millis=$1 var=$2
	local days=$(( total_millis / 1000 / 60 / 60 / 24 ))
	local hours=$(( total_millis / 1000 / 60 / 60 % 24 ))
	local minutes=$(( total_millis / 1000 / 60 % 60 ))
	local seconds=$(( total_millis / 1000 % 60 ))
	local millis=$(( total_millis % 1000 ))
	(( days > 0 )) && human+="${days}d "
	(( hours > 0 )) && human+="${hours}h "
	(( minutes > 0 )) && human+="${minutes}m "
	human+="${seconds}.$(printf "%03d" ${millis})s"
...
prompt_pure_check_cmd_exec_time() {
	integer elapsed
	(( elapsed = EPOCHREALTIME*1000 - ${prompt_pure_cmd_timestamp:-$EPOCHREALTIME}*1000))
	typeset -g prompt_pure_cmd_exec_time=
	(( elapsed > ${PURE_CMD_MAX_EXEC_TIME:-0} )) && {
